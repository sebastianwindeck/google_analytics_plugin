...
class GoogleAnalyticsReportingToMySQLOperator(BaseOperator):
    ...
    :param mysql_conn_id:               The MySQL connection id
    ...
    template_fields = ('since', 'until', 'destination_table')

    def __init__(self, google_analytics_conn_id, view_id, since, until, dimensions, metrics, mysql_conn_id,
                 destination_table, destination_schema=None, if_exists='fail', destination_table_dtypes=None,
                 page_size=1000, include_empty_rows=True, sampling_level=None, dimension_filter_clauses=None,
                 key_file=None, *args, **kwargs):
        ...
        self.metricMap = {'METRIC_TYPE_UNSPECIFIED': 'varchar(255)', 'CURRENCY': 'decimal(20,5)', 'INTEGER': 'int(11)',
            'FLOAT': 'decimal(20,5)', 'PERCENT': 'decimal(20,5)', 'TIME': 'time'}
    ...
    def execute(self, context):
        ...
        report = ga_conn.get_analytics_report(self.view_id, since_formatted, until_formatted, self.sampling_level,
                                              self.dimensions, self.metrics, self.page_size, self.include_empty_rows,
        ...
        dimensionHeaders = [{'name': header.replace('ga:', ''), 'type': 'varchar(255)'} for header in
            columnHeader.get('dimensions', [])]
        metricHeaders = [{'name': entry.get('name').replace('ga:', ''),
                          'type': self.metricMap.get(entry.get('type'), 'varchar(255)')} for entry in
        ...
        df_google_data.to_sql(name=self.destination_table, con=mysql_hook.get_sqlalchemy_engine(),
                              dtype=self.destination_table_dtypes, if_exists=self.if_exists,
